...
class ChineseCheckersBoard(MultiAgentEnv):
    def __init__(self, nplayers):
        ...
        self._observation_spaces = {
            agent: spaces.Box(low=-1, high=2, shape=(width * height,), dtype=np.float32) for agent in self.possible_agents
        }
    ...
    def observation_space(self):
        return spaces.Box(low=-1, high=2, shape=(width * height,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if not self.isLegal(action, self.GlobalBoard, self.current_player):
            ...
            return (np.where((board == -1) | (board == 0), board, np.where(board == self.current_player, 2, 1)).astype(np.float32), rewards, terminateds, {}, {})
        else:
            ...
            if self.isGameOver(board, Token):
                ...
                for x in self.agents[:].pop(self.current_player-1):
                    self.rewards[x] = -10
            ...
            self.next_player()
            return (
                {self.current_player: np.where((board == -1) | (board == 0), board, np.where(board == self.current_player, 2, 1)).astype(np.float32)},
                rewards,
                terminateds,
                {},
                {}
    ...
    def reset(self, *,  seed=None, options=None):
        ...
        board = self.GlobalBoard
        self.num_moves = 0
        self.rewards = {i : 0 for i in range(1,self.numPlayers+1)}
        self.current_player = 1
        # obs = np.where((board == -1) | (board == 0), board, np.where(board == self.current_player, 2, 1)).astype(np.float32)
        obs = {self.current_player: np.where((board == -1) | (board == 0), board, np.where(board == self.current_player, 2, 1)).astype(np.float32)}
        return obs, {}
...
# import random
# import time
# env = ChineseCheckersBoard(3)
# env.reset()
# env.render(env.GlobalBoard)
# while True:
#     possibleMoves = env.allLegalActions(env.GlobalBoard, env.current_player)
#     obs, rewards, terminated, truncated, info = env.step(random.choice(possibleMoves))
