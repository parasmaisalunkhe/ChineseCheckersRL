...
from pettingzoo.test import api_test, render_test, collect_render_results
width = 29
height = 19
...
class ChineseCheckersBoard(AECEnv):
    ...
    def __init__(self, numPlayers, render_mode=None):
        ...
        self.PlayerPOVPosition = {2: [0,3], 3: [0,2,4], 4: [0,1,3,4], 6: [0,1,2,3,4,5]}.get(numPlayers) #CCW
        ...
        self.possible_agents = [str(r) for r in range(1,numPlayers+1)]
    ...
    def nextPlayerPOV(self):
        ...
        board = self.rotateNtimes(self.currentPlayerBoardView, rotationMove)
        ...
        for x in posOneStepMoves:
            if x[0] > 0 and x[0] < width*height:
                if board[x[0]] == 0:
                    ...
                    elif index not in self.ActualEndingLocations and x[0] not in self.ActualEndingLocations:
                        possibleSteps.add(x[0])
                else:
                    if x[1] > 0 and x[1] < width*height and board[x[1]] == 0 and board[x[1]] != -1:
                        ...
                        elif index not in self.ActualEndingLocations and x[1] not in self.ActualEndingLocations:
                            possibleSteps.add(x[1])
    ...
    def step(self, action):
        ...
        board[action[0]] = 0
        board[action[1]] = agent
        startingagent = self.agent_selection
        if self.isGameOver(board, agent):
            self.rewards[agent] = 5
        newBoardPOV = board
        while True:
            # self.render(newBoardPOV)
            nextAgent = self._agent_selector.next()
            newBoardPOV = self.rotate(newBoardPOV)
            possibleMoves = self.allLegalActions(newBoardPOV, int(agent))
            print(possibleMoves)
            self.observations[nextAgent] = {
                "board": np.where((newBoardPOV == -1) | (newBoardPOV == 0), newBoardPOV, np.where(newBoardPOV == int(agent), 2, 1)),
                "action_mask": tuple(possibleMoves)
            }
            if startingagent == nextAgent:
                break
            
        board = self.rotateNtimes(board, rotationMOve)
        self.GlobalBoard = board
        self.num_moves += 1
        # return np.where((board == -1) | (board == 0), board, np.where(board == agent, 2, 1)), -1, False, board
    ...
    def reset(self, seed=None, options=None):
        ...
        while True:
            # self.render(newBoardPOV)
            newBoardPOV = self.GlobalBoard
            nextAgent = self._agent_selector.next()
            newBoardPOV = self.rotate(newBoardPOV)
            possibleMoves = self.allLegalActions(newBoardPOV, int(self.agent_selection))
            # print(possibleMoves)
            self.observations[nextAgent] = {
                "board": np.where((newBoardPOV == -1) | (newBoardPOV == 0), newBoardPOV, np.where(newBoardPOV == int(self.agent_selection), 2, 1)),
                "action_mask": tuple(possibleMoves)
            }
            if self.agent_selection == nextAgent:
                break
        self.num_moves = 0
        """
